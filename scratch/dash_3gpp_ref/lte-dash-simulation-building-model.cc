/* -*-  Mode: C++; c-file-style: "gnu"; indent-tabs-mode:nil; -*- */
  ...
  *
  * Modified by: Sanjay Chawla <schawla@tcd.ie>
  */
           ...
           if (enbdev)
             {
               ...
               outFile << "set label \"" << enbdev->GetCellId ()
                       << "\" at "<< pos.x << "," << pos.y
                      << " left font \"Helvetica,4\" textcolor rgb \"white\" front  point pt 2 ps 0.3 lc rgb \"white\" offset "<<(enbdev->GetCellId()-1)/19 <<",0"
                       << std::endl;
             }
         }
     }
...
static ns3::GlobalValue g_macroEnbBandwidth ("macroEnbBandwidth",
 ...
 static ns3::GlobalValue g_outdoorUeMaxSpeed ("outdoorUeMaxSpeed",
                                              ...
                                              ns3::DoubleValue (0.0),
                                              ns3::MakeDoubleChecker<double> ());
 static ns3::GlobalValue g_dashSegmentDuration ("dashSegmentDuration",
                                               "Duration of a single DASH segment [s].",
                                               ns3::IntegerValue (15),
                                               ns3::MakeIntegerChecker<uint16_t> ());
 int
 main (int argc, char *argv[])
 {
  ...
  std::cout<<"\nStarting...\n";
   ...
   GlobalValue::GetValueByName ("dashSegmentDuration", integerValue);
   int dashSegmentDuration = integerValue.Get ();
 ...
 //   LogComponentEnable ("PfFfMacScheduler", LOG_LEVEL_ALL);
  ...
  // home UEs located in the same apartment in which there are the Home eNBs
   ...
   // output directory based on number of UEs
   std::stringstream outputDirStream;
   outputDirStream << "output/lte/dualstripe/ues" << macroUeDensity*1000000 << "_" << dashSegmentDuration << "s/";
   ...
   if (epc)
     {
       ...
       out_addr_file << remoteHost->GetId() <<  " " << remoteHostAddr << endl;
                   ...
                   if (epcDl)
                     {
                       ...
                       dashPath << "/DashWorkspace/bbb_" << std::to_string(dashSegmentDuration) << "s/";
						ssMPDURL << "http://" << remoteHostAddr << dashPath.str() << "vid1.mpd.gz";
						...
						DASHServerHelper server(Ipv4Address::GetAny(), port,  "1.0.0.2", dashPath.str(), representationStrings, dashPath.str());
						serverApps = server.Install (remoteHost);


						// HttpClientHelper clientHelper(remoteHostAddr, port, "/BigBuckBunny_15s_onDemand_2014_05_09.mpd.gz", "1.0.0.2");
						// clientApps.Add(clientHelper.Install(ue));
						// HttpServerHelper serverHelper(Ipv4Address::GetAny(), port, GetCurrentWorkingDir() + "/../content/segments/BigBuckBunny/bunny_2s/", "1.0.0.2");
						// HttpServerHelper serverHelper(Ipv4Address::GetAny(), port, GetCurrentWorkingDir() + "/../../DashWorkspace/bbb_15s/", "1.0.0.2");
  ...
  if (generateRem)
    {
      ...
      PrintGnuplottableUeListToFile (outputDir + "/ues.txt");
       ...
       remHelper->SetAttribute ("OutputFile", StringValue (outputDir + "lena-dual-stripe.rem"));
   ...
   // Ptr<UniformRandomVariable> dashTracerRand = CreateObject<UniformRandomVariable> ();
   // dashTracerRand->SetAttribute ("Min", DoubleValue (1.0));
   // dashTracerRand->SetAttribute ("Max", DoubleValue (ues.GetN()- 1));
   // for (uint8_t i = 0; i < 6; ++i) {
	   DASHPlayerTracer::Install(ues, DashTraceFile);
   //}
