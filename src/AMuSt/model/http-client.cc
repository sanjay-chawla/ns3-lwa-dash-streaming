          ...
          if (p)
          {
            ...
            if (p)
            {
              ...
              *contentLength = iActualContentLength;
              ...
              NS_LOG_ERROR("ERROR: could not find where body begins\n");
            }
          }
          else
          {
        	  NS_LOG_ERROR("ERROR: No CRLF found?!?\n");
          }

        } else
        {
        	NS_LOG_ERROR("ERROR: Server did not reply Content-Length Header field\n");
        }
      }

    } else
    {
    	NS_LOG_ERROR("Invalid HTTP Response\n");
    }

  } else
  {
	  NS_LOG_ERROR("Not sure what this response header means\n");
	  NS_LOG_ERROR("Result\n");
  }
...
HttpClientApplication::HandleRead (Ptr<Socket> socket)
{
  ...
  while (true)
  {
    ...
    NS_LOG_DEBUG ("packet in application: >>>>>>>>>>>>>>>>>>>>>....." << packet->ToString());
