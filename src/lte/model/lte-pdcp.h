...
public:
  ...
  /**
     *
     *	Sets reordering at UE
     * \param lcId
     */
    void SetUeReordering (bool ueReordering);

    /**
     *
     *
     * Gets reordering at UE
     */
    bool GetUeReordering ();
  ...
  void SetLwaRxSocket (Ptr<Socket> socket);
  ...
  virtual void ReceiveLwaPacket (Ptr<Socket> socket);
...
protected:
  ...
  TracedCallback<LteRlcSapProvider::TransmitPdcpPduParameters >m_pdcptxtrace ;
...
private:
  ...
  static const uint16_t m_reorderingWindow = 2048;

  uint16_t m_rxHfn = 0;

  uint16_t m_lastSubmittedRxSn = m_maxPdcpSn;

  uint16_t m_reorderingRxCount;

  bool ueReordering = false;

  std::map<uint32_t, Ptr<Packet>> m_sdusBuffer;

  Ptr<Socket> m_staRxSocket;
